    ...
    for i in range(n_programs):
        ...
        if sample_weight is None:
            ...
            if isinstance(sample_weight, torch.Tensor):
                curr_sample_weight = sample_weight.clone()
            else:
                curr_sample_weight = sample_weight.copy()
        if isinstance(sample_weight, torch.Tensor):
            oob_sample_weight = curr_sample_weight.clone()
        else:
            oob_sample_weight = curr_sample_weight.copy()
        ...
        if self.feature_names is not None:
            if self.n_features_in_ != len(self.feature_names):
                raise ValueError(
                    ...
                    "length to n_features. Expected %d, got %d." % (self.n_features_in_, len(self.feature_names))
                )
        ...
        for gen in range(prior_generations, self.generations):
            ...
            population = Parallel(n_jobs=n_jobs, verbose=int(self.verbose > 1))(
                delayed(_parallel_evolve)(
                    n_programs[i], parents, X, y, sample_weight, seeds[starts[i] : starts[i + 1]], params
                )
