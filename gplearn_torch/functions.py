...
def _protected_division(x1, x2):
    ...
    return torch.where(torch.abs(x2) > 0.001, torch.divide(x1, x2), 1.0)
...
def _protected_log(x1):
    ...
    return torch.where(torch.abs(x1) > 0.001, torch.log(torch.abs(x1)), 0.0)
...
def _protected_inverse(x1):
    ...
    return torch.where(torch.abs(x1) > 0.001, 1.0 / x1, 0.0)
